      ...
      input.flatMap(async ({ chain, walletAddress, assets }) => {
        ...
        const assetPromises = assets.map(async ({ name, tokenAddress }) => {
          try {
            ...
            const priceData =
              llamaJson.coins?.[`${llamaChain}:${tokenAddress.toLowerCase()}`];
        ...
        // Native ETH balance only for Ethereum chain
        const nativeEthPromise =
          chain === "ethereum"
            ? (async () => {
                try {
                  const nativeBalanceRes = await fetch(baseUrl, {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({
                      jsonrpc: "2.0",
                  ...
                  const hexBalance = nativeData.result;
                  const ethBalance =
                    parseFloat(BigInt(hexBalance).toString()) / 1e18;
                  ...
                  const ethPrice =
                    priceJson.coins["coingecko:ethereum"]?.price || 0;
                  ...
                  return {
                    ...
                    usdPrice: 0,
                    usdValue: 0,
                    error: err.message || "Failed to fetch ETH balance",
                  };
                }
              })()
            : null;
        ...
        const all = await Promise.all([
          ...assetPromises,
          ...(nativeEthPromise ? [nativeEthPromise] : []),
        ]);
